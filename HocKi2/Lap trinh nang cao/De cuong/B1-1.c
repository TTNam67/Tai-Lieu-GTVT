...
void Read(char *Filename, int *n, int *m, float **a, float **b)
{
	...
	int i;
	for (i = 0; i <= *n; i++) fscanf(file, "%f", *a+i);
	for (i = 0; i <= *m; i++) fscanf(file, "%f", *b+i);
	fclose(file);
}
...
void Print(int n, float *a, char Name)
{
	...
	printf("%.2f*x^0", a[0]);
	int i;
	for (i = 1; i <= n; i++) printf(" + %.2f*x^%d", a[i], i);
	printf("\n");
}
...
void Subtract(float *a, int n, float *b, int m)
{
	...
	if (n > m)
	{
		for (i = min(n, m) + 1; i <= max(n, m); i++) c[i]= a[i];
	}
	else 
	{
		for (i = min(n, m) + 1; i <= max(n, m); i++) c[i] = -b[i];
	}
	
	for (i = 0; i <= max(n, m); i++) printf("%.2f ", c[i]);
}
